# utils_asset/report_asset.py

"""
IT Asset Management - Final Report Export (Structured v2)
...
Usage in Streamlit:
    ...
    from utils_asset.report_asset import generate_asset_report
    pdf_bytes, docx_bytes = generate_asset_report(
        df,
        client_name="UEMS",
        period="Jan–Sep 2025",
        logo_path="logo.png",
    ...
    from utils_asset.report_asset import generate_asset_report_fn
    pdf_bytes, docx_bytes = generate_asset_report_fn(
        df,
        client_name="UEMS",
        period="Jan–Sep 2025",
        logo_path="logo.png",
...
def _rebalance_sla_content(modules: List[ModuleSection]) -> List[ModuleSection]:
    ...
    For asset modules, this typically no-ops (no 'SLA' section),
...
def _split_bold_runs(text: str):
    ...
    # If we had an odd number of '**' markers, treat the last segment as non-bold
...
def _compute_overview_kpis(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Build KPI dict for Asset template, mirroring the dashboard logic:

      - Total Assets          (__asset_id__ nunique or row count)
      - In-Use (%)           (share of "__asset_status__" == "in use")
      - Unique Brands        (__brand__ nunique)
      - Expiring ≤60 Days    (warranty_end within [today, today+60])
    """
    d = df.copy()

    # Total assets
    if "__asset_id__" in d.columns:
        try:
            total_assets = int(pd.Series(d["__asset_id__"]).nunique())
        except Exception:
    ...
    # In-use %
    inuse_pct = None
    if "__asset_status__" in d.columns:
        stat = d["__asset_status__"].astype(str).str.lower().str.strip()
        denom = stat.notna().sum()
        if denom > 0:
            inuse_pct = float((stat.eq("in use").sum() / denom) * 100.0)

    # Unique brands
    uniq_brands = None
    if "__brand__" in d.columns:
        try:
            uniq_brands = int(pd.Series(d["__brand__"]).dropna().astype(str).nunique())
        except Exception:
            uniq_brands = None

    # Warranties expiring next 60 days
    expiring_60 = None
    if "warranty_end" in d.columns:
        w_end = pd.to_datetime(d["warranty_end"], errors="coerce")
    ...
    return {
        ...
        "In-Use (%)": inuse_pct,          # numeric; we format later
        "Unique Brands": uniq_brands,
        "Expiring ≤60 Days": expiring_60,
    }
...
def _build_kpi_figure(kpis: Dict[str, Any]) -> "go.Figure":
    ...
    for i, (r, c) in enumerate(grid):
        ...
        fig.add_trace(
            go.Indicator(
                ...
                number={"valueformat": ",.1f" if "In-Use" in t else ",.0f"},
                title={"text": t},
    ...
    fig.update_layout(
        ...
        title={"text": "Executive KPI Summary — IT Asset Management", "x": 0.5},
...
def report_asset(
    ...
    title: str = "IT Asset Management — Executive Insight Report",
) -> Tuple[bytes, Optional[bytes]]:
    """
    Builds a fully-structured research-style report (PDF + DOCX)
    from your ASSET recommendation modules.
    """

    modules_cfg = [
        (
            "utils_asset.recommendation.asset_overview",
            "asset_overview",
            "Asset Overview",
        ),
        (
            "utils_asset.recommendation.asset_hardware",
            "asset_hardware",
            "Hardware Assets",
        ),
        (
            "utils_asset.recommendation.asset_software",
            "asset_software",
            "Software Assets",
        ),
        (
            "utils_asset.recommendation.asset_assignments",
            "asset_assignments",
            "Asset Assignments",
        ),
        (
            "utils_asset.recommendation.asset_lifecycle",
            "asset_lifecycle",
            "Asset Lifecycle",
        ),
    ]
    ...
    for mod_path, fn_name, section_title in modules_cfg:
        try:
            ...
            print(f"[WARN] Asset module failed: {mod_path}.{fn_name} — {e}")
            modules.append(ModuleSection(name=section_title))
            continue

    # Executive KPI Summary (asset version)
    kpis = _compute_overview_kpis(df)

    # Format In-Use% & numeric ints nicely for table/docx
    inuse_val = kpis.get("In-Use (%)")
    if isinstance(inuse_val, (int, float)):
        kpis["In-Use (%)"] = f"{inuse_val:.1f}%"
    elif inuse_val is None:
        kpis["In-Use (%)"] = "N/A"

    for key in ("Total Assets", "Unique Brands", "Expiring ≤60 Days"):
    ...
    # Simple appendix: monthly asset creation volume (if created_time exists)
    ...
    if "created_time" in df.columns:
        ...
        appendices["Monthly Asset Volume (table)"] = monthly[["month", "assets"]]
    ...
    if DOCX_AVAILABLE:
        try:
            ...
            print(f"[WARN] DOCX build failed (asset): {e}")
            docx_bytes = None
...
def build_docx(model: ReportModel) -> io.BytesIO:
    """
    Professional IT Asset Management Report Layout
...
class _AssetReportGeneratorCallable:
    """
...
generate_asset_report = _AssetReportGeneratorCallable()

# Optional: plain function alias, if needed outside caching contexts
def generate_asset_report_fn(*args, **kwargs):
    return report_asset(*args, **kwargs)
