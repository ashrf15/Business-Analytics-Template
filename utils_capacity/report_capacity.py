"""
IT Capacity & Infrastructure Optimization - Final Report Export (Structured v2)
...
Usage in Streamlit:
    ...
    from utils_capacity.report_capacity import generate_capacity_report
    pdf_bytes, docx_bytes = generate_capacity_report(
        df,
        client_name="UEMS",
        period="Jan–Sep 2025",
        logo_path="logo.png",
    ...
    from utils_capacity.report_capacity import generate_capacity_report_fn
    pdf_bytes, docx_bytes = generate_capacity_report_fn(
        df,
        client_name="UEMS",
        period="Jan–Sep 2025",
        logo_path="logo.png",
...
def _para_rich(c: canvas.Canvas, text: str, x: float, y: float, lh: float = 14, fs: int = 10) -> float:
    ...
    for word, is_bold in words_runs:
        ...
        if width + word_w > max_w and line:
            cur_x = x
            for w2, b2 in line:
                c.setFont(_font_name(b2), fs)
                ww = c.stringWidth(w2 + " ", _font_name(b2), fs)
                c.drawString(cur_x, y, w2 + " ")
                cur_x += ww
            y -= lh
            if y < M_B + 40:
                _footer(c)
...
def _docx_add_rich_paragraph(doc, text: str, style_name=None, justify=True, space_after_pt=6):
    ...
    if justify:
        from docx.enum.text import WD_ALIGN_PARAGRAPH as WD_ALIGN_PAR
        p.paragraph_format.alignment = WD_ALIGN_PAR.JUSTIFY
...
def _compute_overview_kpis(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Build KPI dict for Capacity template, mirroring recommendations_capacity:

      - Total Active Assets          (asset_id nunique or row count)
      - Avg CPU Utilization (%)      (avg_cpu_utilization)
      - Avg Memory Utilization (%)   (avg_memory_utilization)
      - Avg Storage Utilization (%)  (avg_storage_utilization)
      - Total Monthly Cost (USD)     (cost_per_month_usd)
    """
    d = df.copy()
    ...
    if "asset_id" in d.columns:
        try:
            total_assets = int(pd.Series(d["asset_id"]).nunique())
        except Exception:
            total_assets = int(len(d))
    else:
        total_assets = int(len(d))
    ...
    avg_cpu = _mean_col("avg_cpu_utilization")
    avg_mem = _mean_col("avg_memory_utilization")
    avg_storage = _mean_col("avg_storage_utilization")
    total_cost = _sum_col("cost_per_month_usd")

    return {
        "Total Active Assets": total_assets,
        "Avg CPU Utilization (%)": avg_cpu,
        "Avg Memory Utilization (%)": avg_mem,
        "Avg Storage Utilization (%)": avg_storage,
        "Total Monthly Cost (USD)": total_cost,
    }
...
def _build_kpi_figure(kpis: Dict[str, Any]) -> "go.Figure":
    """
    Single image with KPI indicators (2x3 grid; one cell unused).
    """
    titles = [
        "Total Active Assets",
        "Avg CPU Utilization (%)",
    ...
    def _clean_val(v):
        if isinstance(v, (int, float)) and pd.notna(v):
            return v
        try:
            vv = float(str(v).replace("%", "").replace("$", "").replace(",", ""))
            return vv
        except Exception:
            return str(v)
    ...
    for i, (r, c) in enumerate(grid):
        ...
        fig.add_trace(
            go.Indicator(
                ...
                number={"valueformat": ",.2f" if "Utilization" in t else ",.0f"},
                title={"text": t},
    ...
    fig.update_layout(
        ...
        title={"text": "Executive KPI Summary — Capacity & Infrastructure", "x": 0.5},
...
def _to_html_bold(text: str) -> str:
    """
    Robustly convert **bold** to <b>…</b> and escape the rest for Paragraph.
    """
...
def report_capacity(
    ...
    title: str = "IT Capacity & Infrastructure — Executive Insight Report",
) -> Tuple[bytes, Optional[bytes]]:
    """
    Builds a fully-structured research-style report (PDF + DOCX)
    from your CAPACITY recommendation modules.
    """

    modules_cfg = [
        (
            "utils_capacity.recommendation_capacity.executive_summary",
            "executive_summary",
            "Executive Summary",
        ),
        (
            "utils_capacity.recommendation_capacity.infrastructure_inventory",
            "infrastructure_inventory",
            "Infrastructure Inventory",
        ),
        (
            "utils_capacity.recommendation_capacity.capacity_utilization",
            "capacity_utilization",
            "Capacity Utilization Metrics",
        ),
        (
            "utils_capacity.recommendation_capacity.capacity_planning",
            "capacity_planning",
            "Capacity Planning",
        ),
        (
            "utils_capacity.recommendation_capacity.resource_efficiency",
            "resource_efficiency",
            "Resource Efficiency Analysis",
        ),
        (
            "utils_capacity.recommendation_capacity.bottleneck_identification",
            "bottleneck_identification",
            "Bottleneck Identification",
        ),
        (
            "utils_capacity.recommendation_capacity.resource_allocation",
            "resource_allocation",
            "Resource Allocation and Rightsizing",
        ),
        (
            "utils_capacity.recommendation_capacity.workload_analysis",
            "workload_analysis",
            "Workload Analysis",
        ),
        (
            "utils_capacity.recommendation_capacity.virtualization_cloud",
            "virtualization_cloud",
            "Virtualization and Cloud Optimization",
        ),
        (
            "utils_capacity.recommendation_capacity.storage_capacity",
            "storage_capacity",
            "Storage Capacity Analysis",
        ),
        (
            "utils_capacity.recommendation_capacity.network_capacity",
            "network_capacity",
            "Network Capacity Analysis",
        ),
        (
            "utils_capacity.recommendation_capacity.energy_efficiency",
            "energy_efficiency",
            "Energy Efficiency and Sustainability",
        ),
        (
            "utils_capacity.recommendation_capacity.cost_benefit",
            "cost_benefit",
            "Cost-Benefit Analysis",
        ),
        (
            "utils_capacity.recommendation_capacity.risk_assessment",
            "risk_assessment",
            "Risk Assessment",
        ),
        (
            "utils_capacity.recommendation_capacity.actionable_insight",
            "actionable_insight",
            "Actionable Insights",
        ),
        (
            "utils_capacity.recommendation_capacity.implementation_plan",
            "implementation_plan",
            "Implementation Plan",
        ),
    ]
    ...
    for mod_path, fn_name, section_title in modules_cfg:
        try:
            ...
            print(f"[WARN] Capacity module failed: {mod_path}.{fn_name} — {e}")
            modules.append(ModuleSection(name=section_title))
            continue

    # Executive KPI Summary (capacity version)
    kpis = _compute_overview_kpis(df)

    # Format values nicely
    if kpis.get("Avg CPU Utilization (%)") is not None:
        kpis["Avg CPU Utilization (%)"] = f"{kpis['Avg CPU Utilization (%)']:.2f} %"
    else:
        kpis["Avg CPU Utilization (%)"] = "N/A"

    if kpis.get("Avg Memory Utilization (%)") is not None:
        kpis["Avg Memory Utilization (%)"] = f"{kpis['Avg Memory Utilization (%)']:.2f} %"
    else:
        kpis["Avg Memory Utilization (%)"] = "N/A"

    if kpis.get("Avg Storage Utilization (%)") is not None:
        kpis["Avg Storage Utilization (%)"] = f"{kpis['Avg Storage Utilization (%)']:.2f} %"
    else:
        kpis["Avg Storage Utilization (%)"] = "N/A"

    if kpis.get("Total Active Assets") is not None:
        try:
            kpis["Total Active Assets"] = int(kpis["Total Active Assets"])
        except Exception:
            pass

    if kpis.get("Total Monthly Cost (USD)") is not None:
        try:
            kpis["Total Monthly Cost (USD)"] = f"${kpis['Total Monthly Cost (USD)']:,.2f}"
        except Exception:
            pass
    else:
        kpis["Total Monthly Cost (USD)"] = "N/A"

    # Appendix: monthly capacity summary if created_time exists
    ...
    if "created_time" in df.columns:
        ...
        if not tmp.empty:
            tmp["month"] = tmp["created_time"].dt.to_period("M").astype(str)

            agg = {}

            if "avg_cpu_utilization" in tmp.columns:
                agg["avg_cpu_utilization"] = lambda s: pd.to_numeric(s, errors="coerce").mean()
            if "avg_memory_utilization" in tmp.columns:
                agg["avg_memory_utilization"] = lambda s: pd.to_numeric(s, errors="coerce").mean()
            if "avg_storage_utilization" in tmp.columns:
                agg["avg_storage_utilization"] = lambda s: pd.to_numeric(s, errors="coerce").mean()
            if "cost_per_month_usd" in tmp.columns:
                agg["cost_per_month_usd"] = lambda s: pd.to_numeric(s, errors="coerce").sum()
            ...
            if agg:
                ...
                appendices["Monthly Capacity & Cost Summary (table)"] = monthly
    ...
    if DOCX_AVAILABLE:
        try:
            ...
            print(f"[WARN] DOCX build failed (capacity): {e}")
            docx_bytes = None
...
def build_docx(model: ReportModel) -> io.BytesIO:
    """
    Professional Capacity & Infrastructure Report Layout
    ...
    if model.kpis:
        ...
        for k, v in model.kpis.items():
            ...
            row[0].text = str(k)
            row[1].text = "N/A" if v is None else str(v)
    else:
...
class _CapacityReportGeneratorCallable:
    """
...
generate_capacity_report = _CapacityReportGeneratorCallable()

# Optional: plain function alias, if needed outside caching contexts
def generate_capacity_report_fn(*args, **kwargs):
    return report_capacity(*args, **kwargs)
