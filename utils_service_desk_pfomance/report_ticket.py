"""
Service Desk Performance - Final Report Export (Structured v2)
...
Usage in Streamlit:
    ...
    from utils_service_desk_pfomance.report_ticket import generate_service_desk_report
    pdf_bytes, docx_bytes = generate_service_desk_report(df, client_name="UEMS", period="Jan–Sep 2025", logo_path="logo.png")

    # If you absolutely need a function symbol:
    from utils_service_desk_pfomance.report_ticket import generate_service_desk_report_fn
    pdf_bytes, docx_bytes = generate_service_desk_report_fn(df, client_name="UEMS", period="Jan–Sep 2025", logo_path="logo.png")
...
class FigureBlock:
    ...
    cio_tables: Dict[str, pd.DataFrame] = field(default_factory=dict) #cio table
...
def _rebalance_sla_content(modules: List[ModuleSection]) -> List[ModuleSection]:
    """If SLA-ish content fell into Resolution, move it to SLA."""
...
def _md_table_to_df(md: str) -> pd.DataFrame:
    ...
    # Clean cell text: turn <br> into an em dash separator and remove stray pipes
    for col in df.columns:
        df[col] = (
            df[col].astype(str)
                # convert HTML line breaks to an em dash separator
                .str.replace("<br>", " — ", regex=False)
                # remove literal pipes
...
def _para_rich(c: canvas.Canvas, text: str, x: float, y: float, lh: float=14, fs: int=10) -> float:
    ...
    for word, is_bold in words_runs:
        ...
        if width + word_w > max_w and line:
            ...
            y -= lh
            if y < M_B + 40:
                _footer(c); c.showPage(); y = PAGE_H - M_T
            line, width = [], 0.0
        line.append((word, is_bold)); width += word_w
...
def _compute_overview_kpis(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Replicates the Overview Metrics exactly like the recommendation tab:
      - Total Tickets
      - Closed Tickets
    ...
    if status_col:
        ...
        closed_count = int(status_series.isin(["closed", "resolved", "completed", "done"]).sum())
        open_count = int(total_tickets - closed_count)

    # Avg resolution time (hrs), numeric-safe
    avg_res_str = "N/A"
    if "resolution_time" in df.columns:
        avg_res = pd.to_numeric(df["resolution_time"], errors="coerce").mean()
        if pd.notna(avg_res):
...
def _build_kpi_figure(kpis: Dict[str, Any]) -> "go.Figure":
    """
    Single image with 5 KPI indicators (2x3 grid, last cell intentionally blank).
    Matches your corporate palette via _save_fig before export.
    """
    titles = [
        "Total Tickets",
        "Closed Tickets",
    ...
    def _clean_val(v):
        # Indicator likes numbers/strings; we keep strings like "N/A" as-is
        if isinstance(v, (int, float)) and pd.notna(v):
            return v
        try:
            # try numeric cast for strings like "123.45"
            vv = float(str(v))
            return vv
        except Exception:
            return str(v)
    ...
    fig = go.Figure()
    # Grid: 2 rows x 3 cols (5 indicators + 1 empty space)
    grid = [
        (0, 0), (0, 1), (0, 2),
        (1, 0), (1, 1),  # (1,2) left empty
    ]

    for i, (r, c) in enumerate(grid):
        if i >= len(titles):  # guard
            break
        t = titles[i]
        v = values[i]
        ...
        if not isinstance(v, (int, float)):
            fig.add_annotation(
                ...
                x=(c + 0.5) / 3, y=(1 - r / 2) - 0.25/2,  # center-ish in the cell
...
def _docx_add_corporate_table(doc, df, header_fill_rgb=(0xD9, 0xD9, 0xD9)):
    ...
    for i, col in enumerate(df.columns):
        ...
        run.bold = True; run.font.color.rgb = BRAND
    for _, row in df.iterrows():
        cells = table.add_row().cells
        for i, col in enumerate(df.columns):
            cells[i].paragraphs[0].add_run(str(row[col]))
...
# ---------- ReportLab: bold-aware paragraph ----------
...
def _to_html_bold(text: str) -> str:
    ...
    # 1) Replace paired **…** with placeholders (multiple pairs supported)
    #    Use a function to avoid overlapping replacements
    def _pair_sub(m):
        inner = m.group(1)
        return "\uE000" + inner + "\uE001"  # placeholders for <b> and </b>

    s = re.sub(r"\*\*(.+?)\*\*", _pair_sub, s)

    # 2) Escape &, <, > so paraparser won't choke on user text
    s = s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")

    # 3) Restore placeholders back to <b> and </b>
    s = s.replace("\uE000", "<b>").replace("\uE001", "</b>")

    # 4) Remove any stray/unmatched ** that were left
    s = s.replace("**", "")
...
def _split_sentences(text: str) -> List[str]:
    ...
    # Don't split bullets or headings here
...
def _draw_analysis_rich(c: canvas.Canvas, raw: str, y: float) -> float:
    ...
    so we avoid ReportLab Paragraph collisions.
    - Respects _tidy_analysis_blocks and _split_sentences.
    - Keeps headings (ending with ':') and bullets ('- ') special.
    """
    for block in _tidy_analysis_blocks(raw):
        for sent in _split_sentences(block):
            ...
            # Heading-style line (e.g. "What this graph is:")
            if sent.endswith(":"):
...
def _draw_para_html(c: canvas.Canvas, html: str, x: float, y: float) -> float:
    # Render a Paragraph (HTML-like) and handle pagination
...
def _draw_table_paginated(c: canvas.Canvas, df: pd.DataFrame, y: float) -> float:
    ...
    Assumes the page is primarily dedicated to the table:
      - If the next chunk doesn't fit, force a new page.
      - Each subsequent chunk goes on its own page.
    """
    if df is None or df.empty:
    ...
    while start < len(rows):
        # Try a reasonable block size; shrink until it fits.
        block = 30
        while block >= 1:
        ...
        # If block collapsed to <1, we had no room on this page at all
        if block < 1:
            # New page & reset y, then try again with full block from same 'start'
            _footer(c)
            c.showPage()
            y = PAGE_H - M_T
            continue

        # More rows remaining? Move to a fresh page for the next chunk.
        if start < len(rows):
            _footer(c)
            c.showPage()
            y = PAGE_H - M_T
...
# ---------- DOCX: write a paragraph with **bold** runs ----------
def _docx_add_bold_runs(doc, text: str):
    ...
    for seg in parts:
        ...
        run.bold = bold
        bold = not bold
    # If unbalanced **, ensure last run not bold
    # (handled by toggling logic; it will finish bold=False on even counts)

# ---------- DOCX: CIO cell writer with bold handling ----------
...
def _para(c: canvas.Canvas, text: str, x: float, y: float, lh: float=14, fs: int=10) -> float:
    ...
    for line in _wrap_text(text, c, CONTENT_W, size=fs):
        ...
        if y < M_B + 40:
            _footer(c); c.showPage(); y = PAGE_H - M_T; c.setFont("Helvetica", fs)
    return y
...
def _df_to_pdf_table(df: pd.DataFrame, zebra=True) -> Table:
    # Local safe wrapper for Paragraph creation
    def _pfrag(txt: str):
        def _mk_para(s: str) -> Paragraph:
            p = Paragraph(s, _P)
            # Pre-wrap once so blPara is set (use a generous width)
            _w, _h = p.wrap(CONTENT_W, PAGE_H)  # any positive width/height works
        ...
        try:
            ...
            # As a last resort, strip any tags and retry
    ...
    # Header + body as Paragraph fragments
    ...
    # --- Force total width to fit CONTENT_W ---
...
def _capture_module(mod, fn_name: str, df: pd.DataFrame, section_title: str) -> ModuleSection:
    ...
    Monkeypatch streamlit + render_cio_tables to capture:
      - overview text (before first chart)
      - per-figure analysis (after each chart until next chart)
      - CIO tables (parsed from markdown)
    """
    section = ModuleSection(name=section_title)
    cio_dicts: Dict[str, str] = {"Cost": "", "Performance": "", "Satisfaction": ""}

    current_fig_idx = -1  # -1 means we're still in "overview"
    ...
    def capture_cio(title, cio):
        ...
        - Attach to the current figure (per-subtopic).
        - Also keep a topic-level aggregation for CIO master.
        """
        # Guard: if no figure yet, treat as topic-level (rare)
        ...
        if isinstance(cio, dict):
            ...
            for k_in, pillar in mapping.items():
                ...
                if not md:
                    continue
                dfp = _ingest(md)
                # Attach to current figure
                if target_fb is not None and not dfp.empty:
                    # If multiple CIO calls per subtopic pillar, append rows
                    if pillar in target_fb.cio_tables and not target_fb.cio_tables[pillar].empty:
                        target_fb.cio_tables[pillar] = pd.concat([target_fb.cio_tables[pillar], dfp], ignore_index=True)
                    else:
                        target_fb.cio_tables[pillar] = dfp
                # Also collect topic-level aggregation to preserve CIO master behavior
                if pillar in section.cio_tables and not section.cio_tables[pillar].empty:
                    section.cio_tables[pillar] = pd.concat([section.cio_tables[pillar], dfp], ignore_index=True)
                else:
    ...
    try:
        ...
        if orig_st is not None: setattr(mod, 'st', orig_st)
        if orig_render is not None: setattr(mod, 'render_cio_tables', orig_render)
...
def report_ticket(
    ...
    title: str="Service Desk Performance — Executive Insight Report"
) -> Tuple[bytes, Optional[bytes]]:
    """Builds a fully-structured research-style report (PDF + DOCX) from your existing modules."""
    modules_cfg = [
        ("utils_service_desk_pfomance.recommendation_performance.ticket_volume",            "ticket_volume",            "Ticket Volume"),
        ("utils_service_desk_pfomance.recommendation_performance.resolution_time",          "resolution_time",          "Resolution Time"),
        ("utils_service_desk_pfomance.recommendation_performance.customer_satisfaction",    "customer_satisfaction",    "Customer Satisfaction"),
        ("utils_service_desk_pfomance.recommendation_performance.technician_performance",   "technician_performance",   "Technician Performance"),
        ("utils_service_desk_pfomance.recommendation_performance.incident_trends",          "incident_trends",          "Incident Trends"),
        ("utils_service_desk_pfomance.recommendation_performance.sla",                      "sla",                      "SLA Performance"),
    ]
    ...
    for mod_path, fn_name, section_title in modules_cfg:
        try:
            ...
            # Keep the SLA header visible even if its module is missing
            if "sla" in mod_path:
                modules.append(ModuleSection(name=section_title))
            # silently skip others
            continue
        except Exception:
            # Same SLA visibility policy on unexpected errors
            if "sla" in mod_path:
                modules.append(ModuleSection(name=section_title))
            continue
    ...
    # Executive KPI Summary — match Overview Metrics from recommendation tab
    kpis = _compute_overview_kpis(df)

    if "resolution_time" in df.columns:
        try:
            kpis["Avg Resolution Time (h)"] = f"{pd.to_numeric(df['resolution_time'], errors='coerce').mean():.2f}"
        except Exception:
            pass
    if "request_status" in df.columns:
        try:
            kpis["Open Tickets"] = f"{(df['request_status'].astype(str).str.lower()=='open').sum():,}"""
        except Exception:
            pass
    if "sla_breach" in df.columns:
        try:
            rate = (df["sla_breach"].astype(str).str.lower().isin(["1","true","yes"])).mean()*100
            kpis["SLA Breach Rate"] = f"{rate:.1f}%"
        except Exception:
            pass
    ...
    if "created_time" in df.columns:
        ...
        monthly = tmp.groupby(tmp["created_time"].dt.to_period("M")).size().reset_index(name="tickets")
...
def build_pdf(model: ReportModel) -> io.BytesIO:
    ...
    # Cover (brand bar + centered title)
    ...
    y = PAGE_H/2 + 80
    c.setFont("Helvetica-Bold", 18)
    c.drawCentredString(PAGE_W/2, y, _sanitize_header(model.title)); y -= 28
    ...
    for line in [t for t in meta if t]:
        c.drawCentredString(PAGE_W/2, y, _sanitize_header(line)); y -= 18
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for idx, module in enumerate(model.modules, start=1):
            ...
            # Visual Insight label
            c.setFont("Helvetica-Bold", 12)
            c.drawString(M_L, y, f"{section_num}.1 Visual Insight")
            y -= 12

            # Optional module overview text before first figure
            # Optional module overview text before first figure (rich text, no Paragraph)
            if module.overview:
            ...
            for i, fb in enumerate(module.figures):
                # New page if too low before subtopic
                if y < 320:
                    _footer(c)
                    c.showPage()
                    y = PAGE_H - M_T
                ...
                # Subtopic heading with numbering
                c.setFont("Helvetica-Bold", 12)
                ...
                try:
                    ...
                    # Page guard before drawing image
                    if y - th < (M_B + 40):
                        _footer(c)
                        c.showPage()
                        y = PAGE_H - M_T

                        # Re-draw module + visual insight headers on new page
                        c.setFont("Helvetica-Bold", 14)
                        c.drawString(M_L, y, f"{section_num}. {module.name}")
                        y -= 18
                        c.setFont("Helvetica-Bold", 12)
                ...
                # ------- ANALYSIS (SINGLE, SENTENCE-PER-LINE, NO Paragraph) -------
                if fb.analysis_paras:
                    # Guard before "Analysis" heading
                    if y < (M_B + 140):
                        _footer(c)
                        c.showPage()
                        y = PAGE_H - M_T
                    ...
                    # Use tidy blocks + sentence splitting, rendered via _para_rich
                    for raw in fb.analysis_paras:
                        y = _draw_analysis_rich(c, raw, y)


                # ------- CIO RECOMMENDATIONS (PER PILLAR, ONE TABLE PER PAGE) -------
                if fb.cio_tables:
                    non_empty = [
                    ...
                    for pillar in non_empty:
                        ...
                        # Start each pillar on a fresh page dedicated to that table
                        _footer(c)
                        c.showPage()
                        y = PAGE_H - M_T
                        ...
                        # Table uses as many pages as needed, but those pages
                        # only contain this table and its continuation.
                        y = _draw_table_paginated(c, dfp, y)

            # Next module
            section_num += 1

    # Appendices: one appendix per page set, using table pagination
    if model.appendices:
...
def build_docx(model: ReportModel) -> io.BytesIO:
    """
    Professional Mesiniaga Report Layout
    ------------------------------------
    Structure:
      Cover → ToC → KPI Summary → Modules
      Each module: Figure + Analysis + CIO tables (Cost/Performance/Satisfaction)
    """
    from docx.shared import RGBColor
    from docx.oxml import OxmlElement
    from docx.oxml.ns import qn
    ...
    def add_analysis_text(paragraphs):
    ...
    # ✅ Use the same front page as PDF
    _docx_add_cover_page(doc, model)

    # ✅ Insert Table of Contents
    _docx_add_toc_placeholder(doc)

    # Start content numbering at 1. Executive KPI Summary
    add_heading("1. Executive KPI Summary", level=1)


    if model.kpis:
        df_kpi = pd.DataFrame({"KPI": list(model.kpis.keys()), "Value": list(model.kpis.values())})
        table = doc.add_table(rows=1, cols=2)
        table.style = "Light Grid Accent 1"
        hdr = table.rows[0].cells
        hdr[0].text = "KPI"; hdr[1].text = "Value"
    ...
    for idx, module in enumerate(model.modules, start=1):
        ...
        # Include module overview with bold-aware runs
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for f_idx, fig_block in enumerate(module.figures):
                ...
                pcap = doc.add_paragraph(cap); pcap.runs[0].italic = True
                pcap.alignment = WD_ALIGN_PARAGRAPH.CENTER

                # --- ANALYSIS tied to this figure (clean & bold-aware) ---
                ...
                # --- CIO Recommendations per subtopic (only if any pillar has rows) ---
                if fig_block.cio_tables:
                    non_empty = [p for p in ("Cost", "Performance", "Satisfaction")
...
class _ReportGeneratorCallable:
    ...
    as an argument to @st.cache_data functions elsewhere.
    """
    def __call__(self, df: pd.DataFrame, client_name: str = "", period: str = "", logo_path: Optional[str] = None):
        return report_ticket(df, client_name=client_name, period=period, logo_path=logo_path)

# Preferred symbol to pass into cached UIs:
generate_service_desk_report = _ReportGeneratorCallable()

# Optional: plain function alias, if needed outside caching contexts
def generate_service_desk_report_fn(*args, **kwargs):
    return report_ticket(*args, **kwargs)
